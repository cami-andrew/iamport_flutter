...
class IamportUrl {
  ...
  IamportUrl(String incomeUrl) {
    ...
    final splittedUrl = url.replaceFirst(RegExp('://'), ' ').split(' ');
    appScheme = splittedUrl[0];
  ...
  bool isAppLink() {
    ...
    if (Platform.isAndroid && appScheme == 'https') {
  ...
  Future<String?> getMarketUrl() async {
    if (Platform.isIOS) {
      ...
      if (package != null) {
        ...
        return UrlData.ANDROID_MARKET_PREFIX + package!;
      }
      switch (appScheme) {
        case UrlData.ISP:
